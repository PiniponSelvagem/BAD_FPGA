#pragma once
    ...
    #define PRINT_K(j, sum, w_idx, w, ip)           printf("        j = %d | sum = %.8f | weights[%d] = %.8f | input[%d] = %.8f\n", j, sum, w_idx, w, j, ip)
    #define PRINT_RK(j, sum, r_idx, r, s)           printf("        j = %d | sum = %.8f | r_weigh[%d] = %.8f | state[%d] = %.8f\n", j, sum, r_idx, r, j, s)

    #define PRINT_Z(i, sum, z)                      printf("  i = %d | sum = %.8f | z[%d] = %.8f\n", i, sum, i, z)
    #define PRINT_R(i, sum, r)                      printf("  i = %d | sum = %.8f | r[%d] = %.8f\n", i, sum, i, r)
    #define PRINT_H(i, sum, h)                      printf("  i = %d | sum = %.8f | h[%d] = %.8f\n", i, sum, i, h)
#else
