...
/* AXI Stream */
// GRU_0
#define INPUT_ARRAY_SIZE_0				(IHEIGHT*FILTERS)
#define INPUT_SIZE_0_IN_BYTES			INPUT_ARRAY_SIZE_0
#define OUTPUT_SIZE_0_IN_BYTES			ALIGN_TO_BYTE((IHEIGHT*GRU_CELLS))	// per direction
#define OUTPUT_SIZE_0_IN_BYTES_BGRU		(OUTPUT_SIZE_0_IN_BYTES*2)			// both directions, end of BGRU_0

// GRU_1
#define INPUT_ARRAY_SIZE_1				((IHEIGHT*GRU_CELLS)*2)
#define INPUT_SIZE_1_IN_BYTES			ALIGN_TO_BYTE(INPUT_ARRAY_SIZE_1)
#define OUTPUT_SIZE_1_IN_BYTES			ALIGN_TO_BYTE((IHEIGHT*GRU_CELLS))	// per direction
#define OUTPUT_SIZE_1_IN_BYTES_BGRU		(OUTPUT_SIZE_1_IN_BYTES*2)			// both directions, end of BGRU_1
...
#define DMA_DEV_ID_STREAM_GRU	XPAR_AXIDMA_0_DEVICE_ID
XAxiDma AxiDmaGru;
/* ******** */
...
void bgru_0(unsigned char* input, unsigned char* output) {
	bgru(
			input, INPUT_SIZE_0_IN_BYTES,
			0, 1,
			OUTPUT_SIZE_0_IN_BYTES,
			INPUT_ARRAY_SIZE_0, FILTERS,
			output, OUTPUT_SIZE_0_IN_BYTES_BGRU
	);
}
void bgru_1(unsigned char* input, unsigned char* output) {

	bgru(
			input, INPUT_SIZE_1_IN_BYTES,
			2, 3,
			OUTPUT_SIZE_1_IN_BYTES,
			INPUT_ARRAY_SIZE_1, GRU_CELLS*2,
			output, OUTPUT_SIZE_1_IN_BYTES_BGRU
	);
}
