...
pathToTestMetadata = './feature_extracted/ff1010bird_metadata.txt'
pathToTestFeature = './feature_extracted/ff1010bird_feature.npy'
...
def trainWithSeed(seed, epochs, save):
    ...
    classes = 2
    opt = Adam(decay = 1e-6)
    x_train, x_test, y_train, y_test = train_test_split(feature, label, test_size=0.2, shuffle=True) # assign validation if needed

    # compile model
    model.compile(loss='binary_crossentropy', optimizer=opt, metrics=['accuracy'])

    #fit the model
    hist = model.fit(x_train, y_train, epochs=epochs, batch_size=32, verbose=2)
    ...
    if save:
        with open(saveModelTo+"all_convnet_seeds.csv", "a") as myfile:
...
"""
for i in range(0, 50):
    trainWithSeed(i, 50, True)
"""

trainWithSeed(35, 50, False)
